...
def get_dataset(path_val, path_test, model_path, device, batch_size, slot=2, T=5):  # 获取所有文件
    ...
    all_file_val = get_allfile(path_val)
...
if __name__ == '__main__':
    ...
    # slot = 3
    # model_path = "./checkpoint/netG_onedmrs_T3.pth"
    slot = 5
    model_path = "./checkpoint/netG_onedmrs_T5.pth"
    # slot = 7
    # model_path = "./checkpoint/netG_onedmrs_T7.pth"

    T: int = 10
    batch_size = 64

    y_val_mse, y_val_nmse, y_test_mse, y_test_nmse = get_dataset(path_val, path_test, model_path, device, batch_size, slot=slot,
                                                         T=T)

    x = floatrange(0, 20, 1.25)
    l1, = plt.plot(x, y_val_nmse, marker='.', color='#1661ab', linestyle='dotted')
    l2, = plt.plot(x, y_test_nmse, marker='.', color='xkcd:orange', linestyle='dotted')
